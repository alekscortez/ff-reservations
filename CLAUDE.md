...
5. **Phase 5 + 5.1 (DONE 2026-05-09)**: web feature parity with the legacy Angular app — layout shell (sidebar + EN/ES toggle) + full CRUD on events / frequent clients (with `tableSettings[]` per-table defaults) / packages / admin users + active-holds list + reservations list + detail (cancel / cash / Square link / SMS / check-in pass) + `staff/reservations-new` (SVG floor-plan picker → hold → customer + payment → confirm) + customer CRM (search OR admin browse-all + name/phone edit + reschedule credits) + admin settings + admin financials (KPI dashboard + receivables + CSV export) + public live map at `/map` + public `/check-in/pass` ticket landing + `/staff/check-in` door scanner (camera + manual). Cash App `/pay` page dropped — Square hosted checkout already exposes Cash App Pay alongside card/Apple/Google.
