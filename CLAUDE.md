...
- **Card on Stand (Square POS handoff)** → `services-square-stand-handoff.mjs` + `<square-stand-handoff>` + `/square-stand-callback` page. **Single-iPad only**: Safari opens `square-commerce-v1://payment/create?data=…`; Square POS swipes on the Stand reader; callback POSTs `/reservations/{id}/payment/square-stand/complete` which resolves order→payment and dispatches `addReservationPayment(method:"square", source:"square-stand")`. Idempotent on `providerPaymentId` (webhook backstop). Handoff rows in `HOLDS_TABLE` under `(PK="STANDPAY")`, 15-min TTL. Tipping + Open Tickets must be OFF in Square POS settings; tenders locked to `CREDIT_CARD`. [[square_stand_handoff_2026_05_17]]
