...
- Still to do: `GET /me/reservations` (needs either a Scan with FilterExpression on `customerCognitoSub` or a new GSI on the `ff-reservations` table — CLAUDE.md author to decide), `DELETE /me` (account deletion via `cognito-idp:AdminDeleteUser`), and a WAF v2 rate-based rule scoped to `/auth/customer/*` to limit OTP-bombing.
