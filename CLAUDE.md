...
> **Branch state (2026-05-16):** Angular 21 SPA in prod. Anonymous public booking live behind `allowAnonymousPublicBooking` flag — customers self-book on `/reserva` (`/map` kept as redirect) → Square checkout → `/r/{id}`. Branded short URLs `famosofuego.com/p/{slug}`. Per-route browser titles + static OG card. "Find my reservation" modal: phone (active hold) or FF-XXXXXX code (any non-CANCELLED). Live-visitor tile on staff dashboard. `/admin/analytics` splits visits/bookings/revenue by source. Layer 2 UTM/fbclid/gclid first-touch attribution. Layer 3 Meta Pixel + CAPI wired but inert until `APP_CONFIG.metaPixelId` + Lambda `META_PIXEL_ID`/`META_CAPI_TOKEN_SECRET_ARN` set. Admin-editable OG images + favicon under "Branding" in `/admin/settings`. Settings sections reordered by tuning frequency. Funnel telemetry FE+BE; `bash scripts/smoke_test_prod.sh` for sanity. Dashboard overhaul 2026-05-16: prominent Take Payment in detail modal; multi-event Recent Activity via `GET /reservations/recent`; staff Reservations default sort = Updated DESC; shared `<take-payment-modal>` (−836 lines from consumers). [[dashboard_payment_overhaul_2026_05_16]]. Memory chains via `[[...]]`. React + Expo port paused on `react` branch (tag `react-port-snapshot-2026-05-09`); don't introduce React/pnpm/Vite/`apps/` on `main`.
...
src/app/                            # core/ (auth/config/layout/http/guards/payments), features/ (public,staff,admin lazy), shared/ (components, ui/ primitives, models)
                                    # shared/components/reservation-detail-modal/ — 4-tab modal (Overview/Payment/Pass/Activity); prominent Take Payment above tabs
                                    # shared/components/take-payment-modal/    — shared cash/credit/Square/Cash App form; parents own service calls
...
- `ff-table-holds` (HOLDS_TABLE — HOLD and RESERVED locks per `(EVENTDATE#YYYY-MM-DD, TABLE#{id})`). Also carries: `(PK="RATE", SK="SMS#…" | "CUSTHOLD#…" | "ANONHOLD#{phoneKey}")` rate-limit rows; `(PK="PRESENCE", SK="SESSION#{sessionId}")` live-visitor presence rows (90s TTL); `(PK="ANALYTICS", SK="VISIT#YYYY-MM-DD#utm_source")` daily visit counters (no TTL, cumulative).
...
- **Card on Stand (Square POS handoff)** → `services-square-stand-handoff.mjs` + `<square-stand-handoff>` + `/square-stand-callback` page. **Single-iPad only**: Safari triggers `square-commerce-v1://payment/create?data=…` URL scheme on the host-stand iPad; Square POS app runs the swipe on the Stand reader; redirects back to `/square-stand-callback?data=…&r=…&returnPath=…`; callback POSTs `/reservations/{id}/payment/square-stand/complete`. BE looks up Order via `getOrderById(transactionId)` → first tender's `payment_id` → `getPaymentById` → `addReservationPayment(method:"square", source:"square-stand", provider:{…})`. Idempotent on `providerPaymentId` (dedupes against webhook backstop). Handoff rows in `HOLDS_TABLE` under `(PK="STANDPAY", SK="HANDOFF#{uuid}")`, 15-min TTL. Mid-flow recovery via `pendingSquareStandPayment` banner (Cancel→`CANCEL_NO_REFUND` only — Square POS owns retry). Tipping must be disabled in Square POS settings or `addReservationPayment` rejects on the cap check. Tenders locked to `CREDIT_CARD`. One-time setup: register callback URL in Square Developer Console → Point of Sale API. [[square_stand_handoff_2026_05_17]]
...
- **Anonymous public booking** (`/public/*`, `/p/{slug}`, `/r/{id}`) → `routes-public-bookings.mjs`. Pre-mint id+token+code+slug. Phone slot `(PK="RATE", SK="ANONHOLD#{phoneKey}")` enforces 1 active unpaid hold/phone; cleared on payment. Flag `allowAnonymousPublicBooking`. Public page `/reserva` (legacy `/map` redirects). [[anon_public_booking_implementation_2026_05_13]]
- **Marketing analytics** → 4 layers: funnel telemetry (`frontend_funnel_event` → CW) · first-touch attribution (`core/analytics/attribution.ts`, persisted to `reservation.attribution`) · `/admin/analytics` (`services-analytics.mjs`, admin-only) · Meta Pixel + CAPI dedup pair (`services-meta-capi.mjs` Graph v23.0; Purchase from webhook with `event_id = purchase_${reservationId}`). [[analytics_dashboards_layers_2_3_2026_05_15]]
- **Live-visitor presence** → `services-presence.mjs` writes 90s-TTL rows to HOLDS_TABLE; `/reserva` heartbeats every 30s while visible; dashboard polls `GET /admin/live-visitors` every 5s.
- **Admin-editable branding** → `services-branding.mjs` + `routes-branding.mjs`. Binary DDB on `(APP, BRANDING#{type})` in `ff-settings`. Public `GET /branding/{filename}` streams bytes (60s cache + ETag) or 302s to baked-in static. Admin `GET/POST/DELETE /admin/branding[/{type}]`. Amplify rewrite `/branding/<*>` → API GW. index.html og:image + favicon SVG point at `/branding/*`. WhatsApp caches per-page-URL ~7d. [[admin_editable_branding_2026_05_16]]
- **Staff lookup by FF-XXXXXX** → `GET /reservations/by-code/{code}` in `routes-reservations-holds.mjs`. Mini-form on /staff/reservations + in-table filter + chip in detail modal.
...
- **Auditing auth** → Auth Model section above + `index.mjs:97-174` helpers.
- **Diagnostic queries** → SMS: `sns/us-east-1/908027422124/DirectPublishToPhoneNumber{,/Failure}`. Cron: `aws logs filter-log-events ... --filter-pattern "scheduled_maintenance"`.
- **iOS-Chrome-only bug** → `/?debug=1` (gated by `localStorage.ff-debug=1`) loads eruda. Disable: `/?debug=0`.
