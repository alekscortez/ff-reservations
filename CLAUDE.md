...
5. **Phase 5 (DONE 2026-05-09)**: web feature parity with the legacy Angular app — layout shell (sidebar + EN/ES toggle) + full CRUD on events / frequent clients / packages / admin users + active-holds list + reservations list + detail (cancel / cash / Square link / Cash App link / check-in pass) + `staff/reservations-new` (table picker → hold → customer + payment → confirm) + customer CRM search/edit + admin settings + public live map at `/map`.
