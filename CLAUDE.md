...
| `HlmDialog` | `<hlm-dialog>` | sizes: `default \| full-on-mobile \| sheet` + `panelClass` + optional `ariaLabel` / `ariaLabelledBy` inputs | All modals. `sheet` for slide-from-edge (topbar quick-actions). Pass `ariaLabel="Title"` for static-title dialogs, or `ariaLabelledBy="some-id"` + give the heading that `id` for dynamic-title dialogs (e.g. reservation-detail-modal where the heading content changes). HlmConfirmDialog already passes its `title` through automatically. |
| `HlmConfirmDialog` | `<hlm-confirm-dialog>` | `[title] [message] [confirmText] [cancelText] [loadingText] [destructive] [loading]` + `(confirm) (cancel)` | Yes/no dialogs. Replaces `window.confirm()` — same shape as Spartan-styled UI. `title` is auto-wired to the wrapped HlmDialog's `aria-label`. Form-prompts still compose `<hlm-dialog>` directly. |
...
- **Signals + `ChangeDetectionStrategy.OnPush` is the default for new feature components.** Migration is ongoing — see memory `signals_onpush_migration_status_2026_05_13.md` for which components are migrated, which are still on default CD (mostly TanStack-using pages — they need the workaround in memory `tanstack_proxy_onpush_reactivity.md`), and the two valid patterns: **bare signals** (`readonly foo = signal(...)`, template `{{ foo() }}`, write `this.foo.set(...)`) for fresh code and **signal-backed accessors** (`private readonly _foo = signal(...)` + `get foo() {…} / set foo(v) {…}`) when an existing public API or spec depends on the property shape. The reservations-new rewrite uses accessors; admin/settings, staff/check-in, public/pay, public/availability, public/check-in-pass use bare signals.
- **Tailwind 3's `.sr-only` is the project's screen-reader-only utility.** Don't roll a custom one. `src/styles.scss` doesn't need to declare it — it's built in. Used today by the table-map keyboard-nav hint paragraph.
- **Empty-state messages on filtered/loaded lists get `aria-live="polite"`** so AT users hear the count change after a filter mutation. Applied across staff/dashboard, staff/reservations, admin/financials (3 sections), admin/events, admin/clients (2026-05-13). Add it to any new filter-driven "No X match…" row.
...
- **Touching `reservations-new.ts`** (staff Hold & Reserve) → orchestration in main file, pure helpers extracted into 5 siblings under `src/app/features/staff/reservations-new/`: `*-utils.ts`, `*-active-hold.ts`, `*-filters.ts`, `*-credits.ts`, `*-confirm.ts`. Each has co-located `*.spec.ts`. The component's own spec is 12 tests covering removeSelectedTable primary-promotion, isCashReceiptRequired matrix, releaseHold empty-entries cleanup, double-fire guard on confirmReservation, and the credit+cash-remainder addPayment roundtrip. Conventions enforced by the 2026-05-13 audit + signals/OnPush rewrite (memories `reservations_new_audit_2026_05_13.md` + `reservations_new_signals_onpush_2026_05_13.md`):
  - **ChangeDetectionStrategy.OnPush is on.** Every template-bound class property is signal-backed via a private `_foo = signal<T>(...)` plus `get foo() { return this._foo(); } / set foo(v) { this._foo.set(v); }` accessor pair. Reads (template `{{ holdId }}`, code `this.holdId`) and writes (`this.holdId = '…'`) keep their pre-rewrite syntax; the getter call inside a reactive context establishes the signal dependency. Don't add new template-bound state as a plain field — wrap it in the same accessor pattern (or use a signal directly for new code).
  - **Caches are `computed()` signals** — `upcomingEventsCache`, `pastEventsCache`, `filteredTablesCache` are `readonly computed<T[]>(...)`. Template invocation form (`filteredTablesCache()`) — memoized so the *ngFor array reference is stable across CD cycles (same iOS Chrome touchend protection as the old imperative cache). The 5 filter FormControls feed the computeds via private `toSignal(filterControl.valueChanges, { initialValue })` shadows. **DO NOT add `recompute*` methods or valueChanges subs that mirror them — that pattern was deleted; computeds auto-recompute.**
  ...
  - **`ngDoCheck` is gone** — the previous syncWorkspaceScrollLock + scheduleDesktopSplitLayout heartbeat moved into a constructor `effect()` that tracks the relevant signals (eventDate / showPastModal / showReservationModal / showReleaseConfirm / desktopSplitHeightPx / compactPanelHeightPx). It runs only on signal change instead of every CD cycle. `ngOnDestroy` still calls `syncWorkspaceScrollLock(true)` explicitly to force-unlock body classes.
  - **`holdCountdownLabel` is a `computed()`** — template invocation form `{{ holdCountdownLabel() }}`. Derives from `_holdCountdown`.
