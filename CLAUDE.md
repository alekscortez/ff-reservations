...
> **Branch state (2026-05-17):** Angular 21 SPA in prod. Anonymous public booking live behind `allowAnonymousPublicBooking` flag — customers self-book on `/reserva` (`/map` redirects) → Square checkout → `/r/{id}`. Branded short URLs `famosofuego.com/p/{slug}`. Per-route titles + static OG card. "Find my reservation" modal: phone (active hold) or FF-XXXXXX code. Live-visitor tile on staff dashboard. `/admin/analytics` splits visits/bookings/revenue by source. Layer 2 UTM/fbclid/gclid first-touch. Layer 3 Meta Pixel + CAPI wired but inert until `metaPixelId` + Lambda Meta env set. Admin-editable OG + favicon under "Branding" in `/admin/settings`. Funnel telemetry FE+BE; `bash scripts/smoke_test_prod.sh`. Dashboard overhaul + shared `<take-payment-modal>` [[dashboard_payment_overhaul_2026_05_16]]. **Card on Stand live (2026-05-17)**: in-venue card swipe via Square POS URL-scheme handoff on the host-stand iPad [[square_stand_handoff_2026_05_17]]. **Change Tables live (2026-05-17)**: staff swap tables on an existing reservation via `<change-table-modal>` from the detail-modal Overview tab; atomic `PUT /reservations/{id}/tables`; bundled cash/credit OR deferred Card on Stand for the delta; CREDIT/REFUND/LEAVE for downgrades; auto pass revoke + reissue + SMS on the PAID branch [[change-tables-2026-05-17]]. Memory chains via `[[...]]`. React + Expo port paused on `react` branch (tag `react-port-snapshot-2026-05-09`); don't introduce React/pnpm/Vite/`apps/` on `main`.
...
- **`@Input() field` read inside `computed()` doesn't react.** Plain `@Input` class fields aren't signal dependencies — `computed(() => this.foo)` captures the value at first run and never re-evaluates on parent updates. Mirror inputs into private signals + sync in `ngOnChanges`, OR migrate to functional `input()`. Hit on `<change-table-modal>` `tables` input (cached empty map → all added tables priced at $0). [[feedback-input-signal-reactivity]]
- **`takeUntilDestroyed()` cancels in-flight cleanup XHRs.** When `onClose()` / `ngOnDestroy()` fires HTTP releases as the component tears down, the pipe synchronously cancels the underlying request before it leaves the browser. `subscribe()` directly without the pipe — HttpClient subscriptions self-complete on response; no leak. Hit on `<change-table-modal>` hold-release cleanup (holds stuck until 5-min TTL). [[feedback-takeuntildestroyed-cancels-cleanup]]
- **API GW HTTP API CORS allowlist doesn't include PATCH.** New mutation routes should use PUT (matches existing `/cancel`, `/payment` style) unless someone goes through the shared-prod CORS update first.
- **Lambda resource policy is capped at 20KB.** Adding new API GW routes piles up per-route invoke statements. Currently ~57 statements; ~150 bytes each. Consolidate stale ones (or switch a noisy route family to a wildcard `source-arn`) before adding new routes when the policy is near full. [[change-tables-2026-05-17]] (the cleanup that made room for the `/tables` route).
...
- **Change Tables (staff swap)** → `services-reservations-table-change.mjs` + `<change-table-modal>` + `PUT /reservations/{id}/tables`. Two-step modal: pick tables (TableMap + delta math + Δ<0 resolution toggles + reason) → (for Δ>0) collect cash/credit OR pick Card on Stand for the deferred path. Backend single-TransactWrite does delete-old + upgrade-new + reservation Update (+ credit Update for method=credit). Post-swap side effects: payment-link deactivation, Δ<0 surplus resolution (CREDIT/REFUND/LEAVE), pass revoke + reissue + SMS on PAID, pass revoke only on deferred. Deferred path chains into `<take-payment-modal>` via parent's `openPayment(..., { defaultMethod: 'square_stand' })`. Hold lifecycle owned by the modal — `createHold` on tile add, `releaseHold` on tile remove or cancel (cleanup `releaseHold` calls must NOT pipe `takeUntilDestroyed` or the XHR gets cancelled on teardown [[feedback-takeuntildestroyed-cancels-cleanup]]). [[change-tables-2026-05-17]]
...
- **Apple Wallet `.pkpass`** → `services-wallet-pass.mjs`; certs in `WALLET_PASS_SECRET_ARN`. `pass.type="generic"`; QR is `ffr-checkin:{64-hex token}` (never the 6-char code); `barcode.altText = FF-{code}`. Installed passes don't auto-refresh; pass content updates (table change, customer rename) require revoke + reissue + re-share. Change Tables auto-fires the revoke + reissue + SMS chain on swap when status stays PAID (deferred path revokes only and lets the follow-up payment trigger reissuance) [[change-tables-2026-05-17]].
