...
- `/me/*` route handlers, CRM merge-by-phone service, `requireCustomerOwnership` helper, `DELETE /me`, `/auth/customer/{start,verify}` backend mediators (so mobile doesn't need to know about the synthetic-email mapping), and WAF rate-based rule on `/auth/customer/*` — still to do; foundation now in place to write them against real customer tokens.
