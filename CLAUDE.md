...
- **Cognito Hosted UI `logout_uri` requires a string-exact LogoutURLs match.** Bare origin doesn't match `${origin}/login` entries; Cognito redirects to `/login?logout_uri=…` where the login endpoint shows "Invalid request". Always build the value as `buildRedirectUrl(APP_CONFIG.cognito.postLogoutPath)` (origin + `/login`); when adding a new host, register `${origin}/login` in LogoutURLs BEFORE shipping a logout flow there. Hit 2026-05-14 (commit `eb5bbda`); spec `auth.service.spec.ts` "logout_uri targets origin + postLogoutPath" is the regression net. Memory: [[cognito_logout_uri_gotcha]].
