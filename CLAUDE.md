...
> **Branch state (2026-05-13 late):** `main` runs the Angular 21 SPA in production with the **anonymous public-booking flow shipped behind `allowAnonymousPublicBooking`** + **Saturday-readiness pass shipped on top**. Customers can self-book on `/map` (with country picker, inline form errors, Turnstile) → Square hosted checkout → land on `/r/{id}` confirmation page (countdown timer on PENDING, self-release button, Apple Wallet badge hidden on Android, "Book another" + "Message us" CTAs on PAID, differentiated copy for auto-released vs paid-but-cancelled CANCELLED states). Short URLs are now `https://famosofuego.com/p/{slug}` (CloudFront/Amplify proxies to `api.famosofuego.com/p/{slug}`). Staff can find any reservation via FF-XXXXXX confirmation code (search-by-code on /staff/reservations + the in-table filter) — code is also rendered as a chip in the detail modal + dashboard urgent-payment cards. Production smoke test at `scripts/smoke_test_prod.sh` (15 checks, ~10s, exits non-zero on fail) — runbook at `[[saturday_operational_runbook]]`. Full anon-booking implementation memo: [[anon_public_booking_implementation_2026_05_13]]; the Saturday-readiness pass arc is in [[public_map_audit_session_2026_05_13_evening]]. A React + Expo monorepo port exists on the `react` branch (snapshot tag `react-port-snapshot-2026-05-09`), paused mid-Phase 5. Do not introduce React, pnpm, Vite, or `apps/`/`packages/` changes on `main`.
...
- **`/p/{slug}` is served by Lambda, but the customer hits the WEB host now.** As of 2026-05-13, an Amplify rewrite rule `/p/<*>` → `https://api.famosofuego.com/p/<*>` (status 200 = server-side proxy) makes `https://famosofuego.com/p/{slug}` work. Lambda env `PUBLIC_BOOKING_SHORT_URL_BASE` is flipped to `https://famosofuego.com` so all customer-facing URLs (Square `customerReturnUrl`, `shortUrl` field, `payment_note` recovery line, slug-based `pass.url`) are branded. The legacy `https://api.famosofuego.com/p/{slug}` keeps working for URLs already in customer inboxes — both routes hit the same Lambda handler. Amplify rewrite rule is in `aws amplify get-app --app-id d1gxn3rvy5gfn4 --query 'app.customRules'`. **Order matters in customRules**: `/p/<*>` must come BEFORE the `/<*>` SPA fallback. The /r URLs that the /p handler 302s to use `PUBLIC_BOOKING_RETURN_BASE_URL` (web host, also famosofuego.com) — keep the two env vars separate even though they happen to point at the same host post-flip; they have different semantic roles.
- **`tsc --noEmit` doesn't run Angular template typecheck.** Bare boolean attributes like `<hlm-confirm-dialog destructive ...>` (and other AOT-strict template errors) pass `tsc --noEmit` cleanly but fail at Amplify's `ng build` (~4 min cycle wasted). Always run `CI=true npm run build` before pushing — same AOT pipeline Amplify uses, surfaces the error in ~10s locally. Bare-boolean fix: write `[destructive]="true"` (the pattern used in `admin/clients/clients.html`). Memory: [[feedback_tsc_misses_template_typecheck]].
...
- **Anonymous public booking (`/public/reservations/*`, `/p/{slug}`, `/r/{id}`)** → entry: `backend/lambda/lib/routes-public-bookings.mjs`. Customer journey: `/map` interactive (HlmDialog + Turnstile widget + country picker + inline form errors) → POST creates hold + reservation + Square hosted link → 302 to Square checkout → Square redirects to `https://famosofuego.com/p/{slug}` (Amplify proxies to API GW) → 302 to `/r/{id}?t=…&eventDate=…` polling page (1Hz countdown timer + Release-hold button on PENDING; on PAID, Apple Wallet badge hidden on Android + "View check-in pass" + "Book another table" + "Message us" CTAs; differentiated CANCELLED card for auto-released vs paid-but-cancelled with WhatsApp/Call recovery). **Pre-mint reservationId + customerToken + confirmationCode + publicSlug upfront** so the same id stamps the phone slot, Square note, and customer-return URL — `createReservation` accepts all four in the payload. Phone-slot registry under `(PK="RATE", SK="ANONHOLD#{phoneKey}")` enforces 1 active unpaid hold per phone (release on payment / explicit release / TTL expiry). Webhook handler (`services-square-payments.mjs:processSquareWebhookEvent`) parses confirmationCode from `payment.note` and resolves via `lookupReservationByConfirmationCode`. Behind `allowAnonymousPublicBooking` settings flag (defaults false). Full implementation memo: [[anon_public_booking_implementation_2026_05_13]]; Saturday-readiness pass arc in [[public_map_audit_session_2026_05_13_evening]].
