...
> **Branch state (2026-05-16):** `main` runs the Angular 21 SPA in prod with anonymous public-booking live behind `allowAnonymousPublicBooking`. Customers self-book on `/reserva` (renamed from `/map` 2026-05-15; `/map` kept as client-side redirect forever — old SMS/Wallet/share links survive) → Square checkout → `/r/{id}`. Branded short URLs `https://famosofuego.com/p/{slug}` via Amplify rewrite. Static index.html title + OG card for Meta ad scraping; per-route titles override on staff/admin. "Find my reservation" modal supports phone (active hold) or FF-XXXXXX code (any non-CANCELLED). Funnel telemetry FE+BE; smoke test `bash scripts/smoke_test_prod.sh`. **Live-visitor tile** on staff dashboard (90s-TTL DDB presence rows). **`/admin/analytics`** dashboard splits visits/bookings/revenue by source. **Layer 2 UTM/fbclid/gclid first-touch attribution** captured + persisted on reservation row. **Layer 3 Meta Pixel + CAPI** wired but inert until `APP_CONFIG.metaPixelId` + Lambda env (`META_PIXEL_ID` + `META_CAPI_TOKEN_SECRET_ARN`) are set. **Admin-editable OG images + favicon** under "Branding" in `/admin/settings` (below the form; uploads land in DDB binary attrs; `/branding/{filename}` Amplify-rewritten to API GW Lambda which streams active bytes or 302s to baked-in static). Settings page sections reordered by tuning frequency (Tier 1: Customer Self-Booking, Payments → Tier 2: Check-In, Public Map, Auto-refresh → Tier 3: Business Hours, Permissions, Connected Services). Full arc in [[anon_public_booking_implementation_2026_05_13]] + [[public_map_audit_session_2026_05_13_evening]] + [[public_map_tier_a_b_session_2026_05_14]] + [[find_by_code_tier_s_2026_05_14]] + [[saturday_operational_runbook]] + [[reserva_url_rename_and_seo_2026_05_15]] + [[analytics_dashboards_layers_2_3_2026_05_15]] + [[admin_editable_branding_2026_05_16]]. React + Expo port paused on `react` branch (tag `react-port-snapshot-2026-05-09`); do not introduce React/pnpm/Vite/`apps/` on `main`.
...
- **Anonymous public booking** (`/public/reservations/*`, `/p/{slug}`, `/r/{id}`, `/public/lookup-by-{phone,code}`, `/public/telemetry`) → `routes-public-bookings.mjs`. Pre-mint id+token+code+slug. Phone slot `(PK="RATE", SK="ANONHOLD#{phoneKey}")` enforces 1 active unpaid hold/phone; cleared on payment. Flag `allowAnonymousPublicBooking`. Public page is `/reserva` (legacy `/map` redirects). [[anon_public_booking_implementation_2026_05_13]] + [[public_map_audit_session_2026_05_13_evening]] + [[public_map_tier_a_b_session_2026_05_14]] + [[find_by_code_tier_s_2026_05_14]] + [[reserva_url_rename_and_seo_2026_05_15]]
- **Marketing analytics** → 4 layers stitched together: (1) funnel telemetry (existing `frontend_funnel_event` → CW); (2) first-touch attribution captured in `src/app/core/analytics/attribution.ts` (utm_*/fbclid/gclid persisted in `localStorage['ff-attribution-v1']`, auto-merged into every telemetry event's extras, threaded into `POST /public/reservations` body, persisted on `reservation.attribution`); (3) `/admin/analytics` dashboard (`services-analytics.mjs` joins DDB counter rows + reservation scan; `GET /admin/analytics?startDate=&endDate=`; admin-only, JWT-gated); (4) Meta Pixel + CAPI dedup pair (`services-meta-capi.mjs` Graph v23.0, `meta-pixel.service.ts` lazy-injects fbevents.js, same `event_id` for ViewContent/InitiateCheckout dedup, Purchase fires only via CAPI from Square webhook with deterministic `event_id = purchase_${reservationId}`). [[analytics_dashboards_layers_2_3_2026_05_15]]
- **Live-visitor presence** → `services-presence.mjs` writes 90s-TTL rows to HOLDS_TABLE; `/reserva` fires `map_heartbeat` every 30s while tab visible; staff dashboard polls `GET /admin/live-visitors` every 5s. Tile uses gentle always-visible indicator (dot + count + popover breakdown).
- **Admin-editable branding** (OG images + favicon for link previews) → `services-branding.mjs` + `routes-branding.mjs`. Binary DDB attribute on `(APP, BRANDING#{type})` rows in `ff-settings`. Public `GET /branding/{filename}` streams active bytes (60s cache + ETag) or 302s to baked-in static fallback. Admin `GET/POST/DELETE /admin/branding[/{type}]` for list/upload/revert. Amplify rewrite `/branding/<*>` → `api.famosofuego.com/branding/<*>`. `index.html` og:image + favicon SVG point at `/branding/*` so admins swap images without redeploy. WhatsApp/iMessage caveat documented in admin UI (their cache is per-page-URL for ~7 days; only manual cache-bust is re-sharing with `?v=...`). [[admin_editable_branding_2026_05_16]]
