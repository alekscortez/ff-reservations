...
- **Signals + `ChangeDetectionStrategy.OnPush` is the default for new feature components.** Two patterns: **bare signals** (`readonly foo = signal(...)`, template `{{ foo() }}`) for fresh code; **signal-backed accessors** when an existing spec depends on the property shape (reservations-new only). TanStack-using pages stay on default CD pending the explicit-input workaround. See memory `signals_onpush_migration_status_2026_05_13.md` for the inventory + recipe.
