...
- Still to do: `GET /me/reservations` (needs either a Scan with FilterExpression on `customerCognitoSub` or a new GSI on the `ff-reservations` table — author to decide) and a WAF v2 rate-based rule scoped to `/auth/customer/*` to limit OTP-bombing.
