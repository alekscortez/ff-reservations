...
- **Card on Stand** (in-venue card swipe): Safari on the host-stand iPad hands the charge off to the Square POS app on the same iPad via the Square POS API URL scheme (`square-commerce-v1://payment/create`). Customer's card swipes natively on the Stand reader; Square POS redirects back to `/square-stand-callback` (full-viewport hero confirmation: green check + amount + booking code + Done) which records the payment automatically and auto-refunds on `/complete` failure or tipping over-capture. Single-iPad only (URL schemes are device-local). iOS Safari often opens the return in a new tab — the wizard tab learns about the payment via a cross-tab `storage` event listener on the just-paid beacon, so the pad flips to "Paid" without a refresh. Requires the callback URL registered in the Square Developer Console + Open Tickets + tipping disabled in Square POS settings (POS API has no flags for those).
...
- Card on Stand recording rejects with `amount cannot exceed remaining balance`: ~~tipping is on in Square POS, customer added a tip, and the captured amount exceeds the deposit cap~~ — as of 2026-05-17 the over-capture path auto-refunds the whole payment + surfaces 409 "Square POS captured more than the deposit (tipping is on?). Disable in Square POS settings and retry." Disable tipping in Square POS settings (`Settings → Checkout → Tips`) so the happy path runs cleanly; the auto-refund is the safety net, not the expected outcome.
- **Card on Stand design iteration** without swiping a real card: visit `/square-stand-callback?preview=done` (also `?preview=error&message=…`, `?preview=cancelled`, `?preview=declined`, `?preview=missing`, `?preview=recording`). Optional `&amount=40&code=K7M3X2` overrides the defaults. Strictly opt-in — production callbacks always carry `?data=…` from Square POS.
