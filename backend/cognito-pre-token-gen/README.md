...
aws lambda create-function \
  --function-name ff-reservations-pre-token-gen \
  --runtime nodejs22.x \
  --role arn:aws:iam::<ACCOUNT_ID>:role/ff-reservations-pre-token-gen-role \
...
The IAM role `ff-reservations-pre-token-gen-role` only needs the basic
