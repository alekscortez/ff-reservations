...
| `services-square-stand-handoff.mjs` | Card on Stand state machine. `startHandoff` writes a 15-min TTL row to `HOLDS_TABLE` under `(PK="STANDPAY", SK="HANDOFF#{uuid}")`; `completeHandoff` resolves the Square POS `transaction_id` → Order → Payment and dispatches `addReservationPayment` with `method: "square"` source `"square-stand"` (idempotent on `providerPaymentId`); `cancelHandoff` marks `PENDING → CANCELLED`. **Auto-refund wired**: `completeHandoff` wraps the `addReservationPayment` dispatch and additionally caps captured amount against `handoff.amount`. Two failure paths refund via `refundSquarePayment` with stable `auto-refund-<paymentId>` idempotency key — record-failure (CCFE, reservation cancelled mid-flow, etc.) and over-capture (Square POS tipping inflated the amount). 409 on refund success, 502 on refund failure (manual reconciliation). History entries (`AUTO_REFUND_AFTER_RECORD_FAILURE` / `AUTO_REFUND_FAILED`, `details.integration: "square-stand"`) flow through the same alarm `ff-res-auto-refund-failed-5m`. Composed in `index.mjs` separately from the barrel (it depends on `getOrderById` + `getPaymentById` + `refundPayment` from `services-square-payments.mjs` + `addReservationPayment` + `getReservationById` + `appendReservationHistory`). |
