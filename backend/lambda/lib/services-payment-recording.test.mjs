...
const TOKEN_HASH = "a".repeat(64); // valid sha-256 hex
...
describe("setReservationCashAppLinkSession validation", () => {
  const baseGood = () => ({
    eventDate: "2026-05-09",
    reservationId: "r1",
  ...
  it("400 on bad eventDate", async () => {
    const { svc } = buildPaymentRecording();
    await assert.rejects(
      () => svc.setReservationCashAppLinkSession({ ...baseGood(), eventDate: "x" }),
      (err) => err?.statusCode === 400
    );
  });

  it("400 on bad tokenHash regex", async () => {
    const { svc } = buildPaymentRecording();
    await assert.rejects(
      () =>
        svc.setReservationCashAppLinkSession({ ...baseGood(), tokenHash: SHORT_HASH }),
      (err) => err?.statusCode === 400 && /SHA-256/.test(err.message)
    );
  });
  ...
  it("400 on amount <= 0", async () => {
    const { svc } = buildPaymentRecording();
    await assert.rejects(
      () => svc.setReservationCashAppLinkSession({ ...baseGood(), amount: 0 }),
      (err) => err?.statusCode === 400
    );
  });

  it("400 on expiresAt <= now", async () => {
    const { svc } = buildPaymentRecording();
    await assert.rejects(
      () =>
        svc.setReservationCashAppLinkSession({ ...baseGood(), expiresAt: FIXED_NOW - 1 }),
      (err) => err?.statusCode === 400
    );
  });

  it("400 when reservation is not CONFIRMED + PENDING/PARTIAL", async () => {
    const { svc } = buildPaymentRecording({
      shared: {
        getReservationById: async () => ({ status: "CANCELLED", paymentStatus: "PENDING" }),
      },
    ...
    await assert.rejects(
      () => svc.setReservationCashAppLinkSession(baseGood()),
      (err) => err?.statusCode === 400
    );
  });

  it("400 when reservation is fully paid", async () => {
    const { svc } = buildPaymentRecording({
      shared: {
        getReservationById: async () => ({
          status: "CONFIRMED",
  ...
  it("400 when amount exceeds remaining balance", async () => {
    ...
    await assert.rejects(
      () =>
        svc.setReservationCashAppLinkSession({ ...baseGood(), amount: 50 }),
      (err) => err?.statusCode === 400 && /remaining balance/i.test(err.message)
    );
  });
});

describe("setReservationCashAppLinkSession happy path", () => {
  it("UpdateCommand sets ACTIVE + token hash + history", async () => {
    const ddb = makeFakeDdb();
    const { svc, historyCalls } = buildPaymentRecording({
      ddb,
      shared: {
    ...
    await svc.setReservationCashAppLinkSession({
      eventDate: "2026-05-09",
      reservationId: "r1",
      tokenHash: TOKEN_HASH,
      amount: 50,
    ...
    assert.equal(update.input.ExpressionAttributeValues[":active"], "ACTIVE");
    assert.equal(update.input.ExpressionAttributeValues[":tokenHash"], TOKEN_HASH);
    assert.equal(update.input.ExpressionAttributeValues[":amount"], 50);
    assert.equal(update.input.ExpressionAttributeValues[":by"], "staff@x");
    assert.match(
      update.input.ConditionExpression,
      /#status = :confirmed AND \(#paymentStatus = :pending OR #paymentStatus = :partial\)/
    ...
    assert.equal(historyCalls.length, 1);
    assert.equal(historyCalls[0].eventType, "CASH_APP_LINK_ISSUED");
    assert.equal(historyCalls[0].details.amount, 50);
  });
});
...
describe("revokeReservationCashAppLinkSession", () => {
  it("returns null on bad eventDate (no DDB call)", async () => {
    const ddb = makeFakeDdb();
    const { svc } = buildPaymentRecording({ ddb });
    const out = await svc.revokeReservationCashAppLinkSession({
      eventDate: "x",
      reservationId: "r1",
    });
  ...
  it("returns null when reservationId is empty", async () => {
    const ddb = makeFakeDdb();
    const { svc } = buildPaymentRecording({ ddb });
    const out = await svc.revokeReservationCashAppLinkSession({
      eventDate: "2026-05-09",
  ...
  it("flips ACTIVE → REVOKED with the right condition", async () => {
    const ddb = makeFakeDdb();
    const { svc } = buildPaymentRecording({ ddb });
    await svc.revokeReservationCashAppLinkSession({
      eventDate: "2026-05-09",
      reservationId: "r1",
      actor: "staff@x",
    });
    ...
    assert.equal(update.input.ConditionExpression, "#cashAppLinkStatus = :active");
    assert.equal(update.input.ExpressionAttributeValues[":revoked"], "REVOKED");
    assert.equal(update.input.ExpressionAttributeValues[":by"], "staff@x");
    assert.match(update.input.UpdateExpression, /REMOVE #cashAppLinkTokenHash/);
  });

  it("swallows ConditionalCheckFailedException → null", async () => {
    const ccfe = new Error("not active");
  ...
  it("propagates non-CCFE errors", async () => {
    const other = new Error("Throughput exceeded");
    other.name = "ProvisionedThroughputExceededException";
    const ddb = makeFakeDdb({ throwOnCommand: { UpdateCommand: other } });
    const { svc } = buildPaymentRecording({ ddb });
    await assert.rejects(
      () =>
        svc.revokeReservationCashAppLinkSession({
          eventDate: "2026-05-09",
          reservationId: "r1",
        }),
...
describe("markReservationCashAppLinkSessionUsed", () => {
  it("returns null on bad input (no DDB call)", async () => {
    const ddb = makeFakeDdb();
    const { svc } = buildPaymentRecording({ ddb });
    const out = await svc.markReservationCashAppLinkSessionUsed({
      eventDate: "garbage",
      reservationId: "r1",
      tokenHash: TOKEN_HASH,
    });
  ...
  it("returns null when tokenHash isn't valid SHA-256 hex", async () => {
    const ddb = makeFakeDdb();
    const { svc } = buildPaymentRecording({ ddb });
    const out = await svc.markReservationCashAppLinkSessionUsed({
      eventDate: "2026-05-09",
      reservationId: "r1",
      tokenHash: SHORT_HASH,
    });
    assert.equal(out, null);
    assert.equal(ddb.calls.length, 0);
  });

  it("flips ACTIVE → USED with both ACTIVE + tokenHash conditions", async () => {
    const ddb = makeFakeDdb();
    const { svc } = buildPaymentRecording({ ddb });
    await svc.markReservationCashAppLinkSessionUsed({
      eventDate: "2026-05-09",
      reservationId: "r1",
      tokenHash: TOKEN_HASH,
      actor: "system:square-webhook",
    });
    const update = ddb.calls[0];
    assert.equal(update.name, "UpdateCommand");
    assert.equal(
    ...
    assert.equal(update.input.ExpressionAttributeValues[":used"], "USED");
    assert.equal(update.input.ExpressionAttributeValues[":tokenHash"], TOKEN_HASH);
    assert.match(update.input.UpdateExpression, /REMOVE #cashAppLinkTokenHash/);
  });

  it("CCFE → null (silent)", async () => {
    const ccfe = new Error("token mismatch");
    ccfe.name = "ConditionalCheckFailedException";
    const ddb = makeFakeDdb({ throwOnCommand: { UpdateCommand: ccfe } });
    const { svc } = buildPaymentRecording({ ddb });
    const out = await svc.markReservationCashAppLinkSessionUsed({
